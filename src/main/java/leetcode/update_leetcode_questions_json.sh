...
QUESTIONS=$(echo "$RESPONSE" | jq '[.stat_status_pairs[] | {frontendQuestionId: .stat.frontend_question_id | tostring, title: .stat.question__title, titleSlug: .stat.question__title_slug, difficulty: (if .difficulty.level == 1 then "Easy" elif .difficulty.level == 2 then "Medium" else "Hard" end)}] | sort_by(.frontendQuestionId | tonumber)')
